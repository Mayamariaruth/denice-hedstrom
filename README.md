...
* Colors consisting of pastel shades of gold/yellow because gold is her favorite color.
...
* I used DevTools to easily move between different screen sizes, simulating sizes between 360px to 1500px.
...
### Fixed Bugs
1. **Issue**
    * @import not working for Google fonts, it gives the error message “Import statements do not load in parallel”.
* **Fix**
    * I added the fonts as a link in the HTML head section instead.

2. **Issue**
    * The navigation bar sits on top of h1 when creating the toggle button.
* **Fix**
    * Removed 'display flex' and 'flex-direction: column' from the header to allow the navbar to sit directly under h1.

3. **Issue**
    * The meditation button's background-color didn't show.
* **Fix**
...
* **Fix**
    * I added a span tag inside the label tag and added an accessibility text. I then added a class to the span and set it to 'Display: none', as well as included 'Aria-hidden="False"' inside the span opening tag to expose the text to assistive technology.
...
- I drew help from [W3Schools](https://www.w3schools.com/html/html_table_borders.asp) for the table on the services page.
